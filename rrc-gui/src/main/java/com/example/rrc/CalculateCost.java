...
public class CalculateCost {
    ...
    // A global counter for the total cost of a raid.
    ...
    public static void calc() {
        Data.reset();   // Resets all data before calculation to make sure nothing gets mixed up.
        resetTotal();
        if (RustCalculator.blockQueue.isEmpty()) {
            // Checks if the queue is empty and replies with the message below.
            System.out.println("The queue is empty! Try adding blocks or deployables to get the cost of a raid.");
        }

        // These blocks of code are altered for each structure, but they server the same purpose.
        Queue<String> aux = new LinkedList<>(RustCalculator.blockQueue);
        // An auxiliary queue is used to easily remove from a duplicate of the original queue and keep the original's integrity.
        while (!aux.isEmpty()) {
            // Loops until the auxiliary queue is empty.
            if (Objects.equals(aux.peek(), "twig block")) {
                // Tests if the current peeked element is equal to the block's structure. In this case it is a twig block.
                // If the element matches the String, the total costs are calculated by dividing the structure's health
                // by the damage that the specific weapon does to the structure. The total is then rounded.
        ...
        // Calls a method to find out the cheapest way to raid the structures in the queue.
        cheapest();
        sceneController.totalCost.setText("Total Cost:" +
        ...
        // Neatly prints the total costs for each weapon/tool.
    }
    public static int sulfurCost;
    public static String tool;
    // Public variables so that the method above can use the values found in the method below.

    // Calculates the cheapest way to raid a structure by multiplying the total amount of weapons/tools by their sulfur
    // costs. Then, a set is used to naturally order the values, least to greatest and an iterator is used to get the
    // first element which is the lowest. Then the element is tested to see which total sulfur cost it matches.
    public static void cheapest() {
        ...
        Set<Double> costsSorted = new TreeSet<>();  // Used to naturally order the values from least to greatest.
        ...
        Iterator<Double> itr = costsSorted.iterator();  // Used to get the first value of the sorted set.
        double least = itr.next();
        sulfurCost = (int) Math.round(least);
        // Rounds the value to an integer and compares it to the sulfur costs to see which tool matches.
